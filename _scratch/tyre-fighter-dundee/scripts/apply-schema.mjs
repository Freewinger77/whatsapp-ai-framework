...
const sql = readFileSync(resolve(root, "supabase/dundee_inbox.sql"), "utf8");
...
for (const connectionString of urls) {
  ...
  try {
    ...
    const tables = await client.query(
      `select tablename from pg_tables where schemaname='public' and tablename like 'dundee_%' order by 1`
    );
    console.log(
      "tables",
