#!/usr/bin/env bash
# Deploy proxy quick-attach (trust detach + catalog APIs + chip UI) to shared workers.
#
# SAFETY:
...
#
# Usage:
#   bash deploy/scripts/deploy-proxy-quick-attach.sh
#   ONLY=wasup3 bash deploy/scripts/deploy-proxy-quick-attach.sh
#
set -uo pipefail
...
node --check "$APP/src/utils/proxy-pool.js" || exit 1
...
deploy_host() {
  ...
  local tmp="/tmp/wasup-proxy-qa-$$"
  ssh -o ConnectTimeout=20 "$user@$host" "mkdir -p '$tmp/src/utils' '$tmp/public'" || return 1

  scp -q -o ConnectTimeout=20 "$APP/src/utils/proxy-pool.js" "$user@$host:$tmp/src/utils/proxy-pool.js" || return 1
...
set -euo pipefail
cp "$tmp/src/utils/proxy-pool.js" "$dir/src/utils/proxy-pool.js"
...
node --check "$dir/src/utils/proxy-pool.js"
cd "\$(dirname "$dir")"
pm2 reload "$pm2"
