...
import { CallbacksMobile } from "@/components/dashboard-mobile";
import { RecordsTable } from "@/components/records-table";
import type { EnquiryRow } from "@/lib/dashboard-model";
...
export default function EnquiriesPage() {
  ...
  const [mobileHours, setMobileHours] = useState<"out" | "all" | "in">("out");
  const [allLeads, setAllLeads] = useState<EnquiryRow[]>([]);
  const extra = [
  ...
  useEffect(() => {
    void fetch("/api/enquiries?limit=500")
      .then((r) => r.json())
      .then((d) => setAllLeads(d.enquiries || []));
  }, []);
  ...
  return (
    <>
        ...
        <RecordsTable<Row>
          ...
          columns={[
            ...
            { key: "phone", label: "Phone", render: (r) => <span className="phone">{r.phone || "—"}</span> },
            {
              key: "channel",
              label: "Channel",
              render: (r) => (r.channel === "phone" ? "Phone" : "Email"),
            },
            { key: "hours", label: "Hours", render: (r) => (r.in_hours ? "Store hours" : "After hours") },
      ...
      <div className="mob-only flush-mob" style={{ height: "100%" }}>
        <CallbacksMobile enquiries={allLeads} hours={mobileHours} setHours={setMobileHours} />
      </div>
    </>
  );
}
