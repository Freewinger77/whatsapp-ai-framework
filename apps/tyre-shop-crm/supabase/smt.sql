...
create table if not exists public.smt_bookings (
  id uuid primary key default gen_random_uuid(),
  smt_id text not null unique,
  customer_smt_id text,
  customer_name text,
  ...
  status_norm text,
  site_status text,
  created_at_smt timestamptz,
  fitting_at timestamptz,
  in_hours boolean not null default false,
  order_total numeric,
  currency text,
  tyre_qty integer not null default 0,
  services text,
  tags text,
  raw jsonb,
  first_seen_at timestamptz not null default now(),
...
create index if not exists smt_bookings_created_idx on public.smt_bookings (created_at_smt desc);
create index if not exists smt_bookings_status_idx on public.smt_bookings (status_norm);
create index if not exists smt_bookings_customer_idx on public.smt_bookings (customer_key);
...
alter table public.smt_bookings enable row level security;
...
revoke all on public.smt_bookings from anon, authenticated;
...
grant all on public.smt_bookings to service_role;
