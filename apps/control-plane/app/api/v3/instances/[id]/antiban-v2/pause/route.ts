...
import { pauseWorkerAntibanV2 } from '../../../../../../../lib/worker-client';
...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const workerBody = await pauseWorkerAntibanV2(workerRequestInput(target, id));
    return NextResponse.json({
      success: true,
      antibanV2: unwrapWorkerAntibanV2(workerBody),
      message: 'Anti-ban v2 paused — sends blocked until resumed.'
    });
  } catch (error) {
